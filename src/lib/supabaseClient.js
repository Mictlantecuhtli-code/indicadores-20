...
export async function getYearsAvailableLuces() {
  const { data, error } = await supabase.from('mediciones_luces_detalle').select('anio');
...
export async function getPistasAvailableLuces() {
  const { data, error } = await supabase.from('mediciones_luces_detalle').select('pista');
...
export async function getCapturasFaunaResumen({ year } = {}) {
  ...
  for (const relation of relations) {
    ...
    const { data, error } = await query.order('anio', { ascending: true }).order('mes', { ascending: true });
  ...
  for (const relation of relations) {
    ...
    if (!error) {
      const years = Array.isArray(data)
        ? data.map(item => Number(item.anio)).filter(yearValue => Number.isFinite(yearValue))
        : [];
...
export async function getYearsAvailableMantenimientos() {
  const { data, error } = await supabase.from('mantenimientos_pavimentos').select('anio');
...
export async function getYearsAvailableDisponibilidad() {
  const { data, error } = await supabase.from('disponibilidad_pistas').select('anio');
...
export async function getYearsAvailablePCI() {
  const { data, error } = await supabase.from('pci_mediciones').select('anio');
