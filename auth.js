...
(function () {
  ...
  const ROLES = (window.ROLES) || { ADMIN:'administrador', SUBDIRECTOR:'subdirector', CAPTURISTA:'capturista' };
  ...
  function setSession(token, userObj, remember) {
    ...
    } catch (e) { err('Error guardando sesión:', e); }
  }
  ...
  function clearSessionEverywhere() {
    ...
    } catch (e) { err('Error limpiando sesión:', e); }
  }

  function parseUser(userStr) { if (!userStr) return null; try { return JSON.parse(userStr); } catch { return null; } }
  function getStoredUser() { const { userStr } = readSession(); return parseUser(userStr); }
  ...
  function permisosPorRol(rol) {
    ...
    if (r === ROLES.ADMIN || r === ROLES.SUBDIRECTOR) {
      return {
        puede_capturar: true, puede_editar: true, puede_ver_historico: true,
    ...
    return {
      puede_capturar: true, puede_editar: false, puede_ver_historico: false,
  ...
  async function login(username, password, remember) {
    ...
    // Backend Supabase si existe (ajusta a tu esquema REAL si lo usas)
    if (window.sb && typeof sb.from === 'function') {
      log('[AUTH] Autenticando contra Supabase...');
      // EJEMPLO (NO usar password plano en producción)
      ...
      if (error) { err('[AUTH] Supabase error:', error); throw new Error('No fue posible autenticar en este momento.'); }
    ...
    // Fallback: USUARIOS local
    log('[AUTH] Backend no disponible → modo local (USUARIOS)');
    if (!Array.isArray(window.USUARIOS)) throw new Error('No hay backend de autenticación ni catálogo local (USUARIOS).');
  ...
  // Escrituras: capturista solo mes vigente del año actual
  function validarOperacionEscritura(operacion, ctx) {
    try {
      ...
      return true;
    } catch (e) { err('[AUTH] validarOperacionEscritura error:', e); return false; }
  }

  function restaurar() {
    const { token, userStr } = readSession();
    if (!token || !userStr) { warn('[AUTH] restaurar: no hay sesión'); return { valid: false, user: null }; }
    const user = parseUser(userStr);
    if (!user) { warn('[AUTH] restaurar: user inválido'); return { valid: false, user: null }; }
  ...
  window.authSystem = { login, logout, getUser: getStoredUser, isAuthenticated, validarOperacionEscritura, restaurar };
